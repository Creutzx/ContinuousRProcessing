...
src_file <- "//deqlab1/Vol_Data/umpqua/RefTempProject/4rUmqRefT2005data_2018_0122_1700.xls"

SubID <- '0124' # Enter the submission ID from VolWQDB
...
for (i in seq_along(logchar$log)) {
  ...
  capture.output(tmp_data <- read_excel(src_file, sheet = logchar[[i,1]], skip = 4, col_types = c("guess", "guess", "numeric", "text" )), file = "nul")
  ...
  cols <- sapply(dr_info, is.logical)
  dr_info[,cols] <- lapply(dr_info[,cols], as.numeric)
